...
export default function PrintPage() {
    ...
    if (!currentDeck) {
        return <div className="min-h-screen flex items-center justify-center">No deck found.</div>;
    }

    // Calculate available space
    const contentWidth = PRINT_CONFIG.PAPER.WIDTH_MM - (2 * PRINT_CONFIG.PAPER.MARGIN_MM);
    const contentHeight = PRINT_CONFIG.PAPER.HEIGHT_MM - (2 * PRINT_CONFIG.PAPER.MARGIN_MM);
        ...
        <div className="min-h-screen bg-slate-100 p-8 print:p-0 print:bg-white font-sans">
            ...
            <div className="fixed top-0 left-0 right-0 h-16 bg-slate-900 text-white flex items-center justify-between px-8 shadow-lg print:hidden z-50">
                <div className="flex items-center gap-4">
                    ...
                    <span className="text-slate-300">{currentDeck.name} ({currentDeck.cards.length} cards)</span>
                </div>
                <div className="flex items-center gap-4">
                    <button
